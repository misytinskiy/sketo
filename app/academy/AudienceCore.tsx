...
type AudienceCoreProps = {
  label: string;
  title: string;
};

export default function AudienceCore({ label, title }: AudienceCoreProps) {
  ...
  return (
    ...
    >
      <span ref={fillRef} className={styles.audienceCoreFill} aria-hidden="true" />
      <div ref={contentRef} className={styles.audienceCoreContent}>
        <span className={styles.audienceCoreLabel} data-reveal="eyebrow">
          {label}
        </span>
        <p className={styles.audienceCoreTitle} data-reveal="headline">
          {title}
        </p>
      </div>
    </div>
